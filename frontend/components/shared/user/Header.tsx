...
const Header = ({ sidebarOpen, setSidebarOpen }: { sidebarOpen: boolean, setSidebarOpen: React.Dispatch<React.SetStateAction<boolean>> }) => {
    return (
        <header className="sticky top-0 z-[999] flex w-full bg-gray-800 lg:rounded-lg overflow-hidden drop-shadow-1">
            <div className="flex flex-grow items-center justify-between py-4 px-4 shadow md:px-6 2xl:px-11 relative before:absolute before:bottom-0 before:left-0 before:w-full before:h-px before:bg-gradient-to-l before:from-sky-400 before:to-emerald-400">
                <div className="flex items-center gap-3 sm:gap-4 lg:hidden">
                    ...
                    <Link href='/user' className="flex flex-shrink-0 lg:hidden items-center bg-gradient-to-r from-sky-400 to-emerald-400 text-transparent bg-clip-text gap-1">
                        <SiStreamrunners className="md:text-4xl text-3xl text-sky-400" />
                        <span className=" font-belanosima md:text-xl text-lg">Classmate+</span>
                    </Link>
                </div>
                ...
                <div className="flex items-center gap-3 2xsm:gap-7">
                    {/* <!-- User Area --> */}
                    <Button className="text-gray-200 text-sm font-barlow px-4 py-2 flex justify-center items-center gap-1 bg-sky-600 hover:bg-emerald-500">
                        Connect
                    </Button>
                    {/* <!-- User Area --> */}
                </div>
